...
public class CurseForgeImporter : IProfileImporter
{
    ...
    public async Task<ImportedProfileContainer> ExtractAsync(CompressedProfilePack pack)
    {
        ...
        return new ImportedProfileContainer(new Profile(manifest.Name, new Profile.Rice(pack.Reference is not null ? PackageHelper.ToPurl(pack.Reference) : null, manifest.Minecraft.Version, LoaderHelper.ToLurl(loader.Identity, loader.Version), manifest.Files.Select(x => PackageHelper.ToPurl(CurseForgeService.LABEL, null, x.ProjectId.ToString(), x.FileId.ToString())).ToList(), new List<string>(), new List<string>()), new Dictionary<string, object>()), pack.FileNames.Where(x => x.StartsWith(manifest.Overrides) && x != manifest.Overrides && x.Length > manifest.Overrides.Length + 1).Select(x => (x, x[(manifest.Overrides.Length + 1)..])).ToList(), pack.Reference?.Thumbnail);
    }

    #endregion

    private bool TryExtractLoader(IEnumerable<ManifestModel.MinecraftModel.ModLoaderModel> loaders, out (string Identity, string Version) loader)
    {
