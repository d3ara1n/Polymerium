    ...
    public async Task<ImportedProfileContainer> ExtractAsync(CompressedProfilePack pack)
    {
        ...
        return new ImportedProfileContainer(new Profile(manifest.Name, new Profile.Rice(pack.Reference is not null
            ...
            pack.FileNames.Where(x =>
                ...
                .Select(x => (x, x[(manifest.Overrides.Length + 1)..])).ToList(), pack.Reference?.Thumbnail);
    }

    private bool TryExtractLoader(IEnumerable<ManifestModel.MinecraftModel.ModLoaderModel> loaders,
