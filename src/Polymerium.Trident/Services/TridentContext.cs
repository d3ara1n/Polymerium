...
public class TridentContext(string home)
{
    ...
    public string InstanceArtifactPath(string key) => Path.Combine(InstanceDir, key, FILE_ARTIFACT);

    public string InstanceProfilePath(string key) => Path.Combine(InstanceDir, $"{key}.json");

    public string AssetIndexPath(string index) => Path.Combine(AssetDir, "indexes", $"{index}.json");

    public string AssetObjectPath(string hash) => Path.Combine(AssetDir, "objects", hash[..2], hash);

    public string NativeDirPath(string key) => Path.Combine(InstanceHomePath(key), "natives");
}