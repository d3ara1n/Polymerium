    ...
    IHttpClientFactory clientFactory)
{
        ...
        JavaHomeLocatorDelegate javaHomeLocator)
    {
        ...
        var tracker = new DeployTracker(key,
            ...
            t =>
            {
                ...
                if (t is { State: TrackerState.Finished }) Launch(key, launch, javaHomeLocator);
            });
        ...
        JavaHomeLocatorDelegate javaHomeLocator)
    {
        ...
        {
            if (tracker.Token.IsCancellationRequested) break;
        ...
        JavaHomeLocatorDelegate javaHomeLocator)
    {
        ...
        if (options.Account == null) throw new InvalidOperationException("Account is not provided");
        ...
        if (found)
        {
            ...
            if (artifact == null) throw new InvalidOperationException("Artifact is not valid");
            ...
            try
            {
                ...
                foreach (var additional in options.AdditionalArguments.Split(' ')) igniter.AddJvmArgument(additional);


                if (options.Mode == LaunchMode.Debug) igniter.Debug();
                ...
                if (options.Mode == LaunchMode.Managed)
                {
                    ...
                    {
                        if (!tracker.IsDetaching) process.Kill();
                    }
                    else
                    {
        ...
        string? vid)
    {
        ...
        if (container.IconUrl is not null) await ExtractIconFileAsync(key.Key, container, client).ConfigureAwait(false);
        ...
        string vid)
    {
        ...
        if (container.IconUrl is not null) await ExtractIconFileAsync(key, container, client).ConfigureAwait(false);
