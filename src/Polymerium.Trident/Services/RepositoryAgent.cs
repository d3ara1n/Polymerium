    ...
    TridentContext context)
{
    ...
    private IRepository OfRepository(string label) => repositories.FirstOrDefault(x => x.Label == label) ?? DUMMY;
    ...
    public async Task<Project> QueryAsync(string label, string projectId,
        CancellationToken token = default) =>
        await RetrieveCachedResultAsync(Path.Combine(context.MetadataDir, label, $"{projectId}.json"), null,
