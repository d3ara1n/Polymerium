    ...
    CancellationToken token = default) : TrackerBase(key, handler, onCompleted, token)
{
    ...
    internal void OnFired() => Fired?.Invoke(this);


    internal void OnDropped(Scrap scrap) => Output?.Invoke(this, scrap);

    internal void OnExited(int code) => Exited?.Invoke(this, code);
}