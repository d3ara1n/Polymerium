...
public class SolidifyManifestStage(ILogger<SolidifyManifestStage> logger, IHttpClientFactory factory) : StageBase
{
    ...
    protected override async Task OnProcessAsync(CancellationToken token)
    {
        ...
        foreach (var fragile in manifest.FragileFiles) files.Add(fragile);

        foreach (var present in manifest.PresentFiles) files.Add(present);

        foreach (var persistent in manifest.PersistentFiles) files.Add(persistent);
        ...
        if (!Path.Exists(buildDir)) Directory.CreateDirectory(buildDir);
        ...
        bool skip = false)
    {
        ...
        if (skip || archive.Entries.Count == 0) return false;
        ...
        foreach (var entry in entries)
        {
            ...
            else if (rootItem != rootDirName) return false;
        }
