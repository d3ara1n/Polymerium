    ...
    internal class ArgumentsItemConverter : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if (token.Type == JTokenType.String)
            {
                ...
                {
                    Values = new string[] { token.ToObject<string>() }
                };
            }
            else if (token.Type == JTokenType.Object)
            {
                ...
                {
                    item.Values = values.Values().Select(x => x.Type == JTokenType.String ? x.Value<string>() : throw new InvalidCastException(x.ToString()));
                }
                else if (value.Type == JTokenType.String)
                {
                    item.Values = new string[] { value.Value<string>() };
                }
                else
                {
