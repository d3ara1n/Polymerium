...
public static class MicrosoftAccountHelper
{
    ...
    private static readonly WapooOptions _options = new() { IgnoreMediaTypeCheck = true };

    // 日后登录部分
    public static async Task<
        Option<MicrosoftAuthenticationResponse>
    > AcquireMicrosoftTokenByDeviceCodeAsync(
    ...
    )
    {
        ...
        await Wapoo
            .Wohoo(DEVICE_ENDPOINT, _options)
            .WithBody(
            ...
            .FetchAsync(token);
        if (deviceCode == null)
            return Option<MicrosoftAuthenticationResponse>.None();
    ...
    public static async Task<Option<XboxLiveResponse>> AcquireXboxTokenAsync(
    ...
    )
    {
        ...
        return response != null
            ? Option<XboxLiveResponse>.Some(response.Value)
            : Option<XboxLiveResponse>.None();
    }

    public static async Task<Option<XboxLiveResponse>> AcquireXstsTokenAsync(
        string xboxToken,
        CancellationToken token = default
    )
    {
    ...
    public static async Task<Option<MinecraftAuthenticationResponse>> AcquireMinecraftTokenAsync(
        string xstsToken,
        string userHash,
        CancellationToken token = default
    )
    {
        ...
        await Wapoo
            .Wohoo(MINECRAFT_ENDPOINT, _options)
            .WithJsonBody(
                new Dictionary<string, string>
                {
    ...
    public static Task<bool> VerifyMinecraftOwnershipAsync(
        string accessToken,
        CancellationToken token = default
    )
    {
    ...
    public static async Task<Option<MinecraftProfileResponse>> GetProfileByAccessTokenAsync(
        string accessToken,
        CancellationToken token = default
    )
    {
        ...
        await Wapoo
            .Wohoo(PROFILE_ENDPOINT, _options)
            .UseBearer(accessToken)
            .ViaGet()
    ...
    public static async Task<Option<MicrosoftAuthenticationResponse>> RefreshAccessTokenAsync(
        string refreshToken,
        CancellationToken token = default
    )
    {
