...
/// <remarks>
///     <para>
///         这是「订阅 + 解析 + 路由」的薄层，<b>不持冗余状态表</b>。状态的唯一真源仍是
///         <see cref="InstanceManager" /> 的 active tracker 字典；Snapshot 由「事件 + 实时查 IsTracking」即时合成。
///     </para>
...
public class InstanceStateAggregator
{
    ...
    public IObservable<InstanceStateSnapshot?> Watch(string key) =>
        Observable
           .Defer(() =>
            {
                var current = _instanceManager.IsTracking(key, out var tracker) ? ToSnapshot(tracker) : null;
