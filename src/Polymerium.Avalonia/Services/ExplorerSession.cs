...
// explorer 只通过这个宿主中性的契约与 session 对话。任何 Trident 持久化类型（如
...
public abstract class ExplorerSession
{
    ...
    // 把一个 exhibit 的 State 还原到提交基线，从 session 自带的数据现算（无需在模型上缓存基线字段）
    public abstract void RevertState(ExhibitModel exhibit);

    // 打开宿主专属的详情弹窗。modifyPending 与 findExisting 是 explorer 传入的两个回调：前者
    ...
    // 提交动作由 session 自声明而非契约方法：Primary 是主动作，Secondary 是挂在主动作
