    ...
    ILogger<GarbageCollector> logger)
{
    public void Execute(IProgress<double?> progress)
    {
        ...
        if (orphanSelectorKeys.Count > 0)
            freeSql.Delete<PersistenceService.AccountSelector>()
                .Where(x => orphanSelectorKeys.Contains(x.Key)).ExecuteAffrows();
        var danglingSelectorUuids = freeSql.Select<PersistenceService.AccountSelector>().ToList(x => x.Uuid)
            .Where(u => !activeUuids.Contains(u)).ToList();
        if (danglingSelectorUuids.Count > 0)
            freeSql.Delete<PersistenceService.AccountSelector>()
                .Where(x => danglingSelectorUuids.Contains(x.Uuid)).ExecuteAffrows();
        progress.Report((double)++completed / totalSteps);

        var orphanActionKeys = freeSql.Select<PersistenceService.Action>().ToList(x => x.Key)
            .Distinct().Where(k => !activeKeys.Contains(k)).ToList();
        if (orphanActionKeys.Count > 0)
            freeSql.Delete<PersistenceService.Action>()
                .Where(x => orphanActionKeys.Contains(x.Key)).ExecuteAffrows();
        progress.Report((double)++completed / totalSteps);

        var orphanActivityKeys = freeSql.Select<PersistenceService.Activity>().ToList(x => x.Key)
            .Distinct().Where(k => !activeKeys.Contains(k)).ToList();
        if (orphanActivityKeys.Count > 0)
            freeSql.Delete<PersistenceService.Activity>()
                .Where(x => orphanActivityKeys.Contains(x.Key)).ExecuteAffrows();
        progress.Report((double)++completed / totalSteps);

        var orphanViewStateKeys = freeSql.Select<PersistenceService.ViewState>().ToList(x => x.Key)
            .Where(k => !activeKeys.Contains(k)).ToList();
        if (orphanViewStateKeys.Count > 0)
            freeSql.Delete<PersistenceService.ViewState>()
                .Where(x => orphanViewStateKeys.Contains(x.Key)).ExecuteAffrows();
        progress.Report((double)++completed / totalSteps);

        var orphanWidgetKeys = freeSql.Select<PersistenceService.WidgetLocalSection>().ToList(x => x.Key)
            .Distinct().Where(k => !activeKeys.Contains(k)).ToList();
