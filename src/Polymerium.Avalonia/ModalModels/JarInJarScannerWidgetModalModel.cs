...
) : ViewModelBase
{
    ...
    private async Task ScanAsync()
    {
        ...
        try
        {
            var (jarCount, entries, topLevelIds) = await Task.Run(
                () => ScanImpl(progress, token), token);
            ...
            {
                entry.Duplicate =
                    innerDuplicates.Contains(entry.ModId) ? HiddenModEntry.DuplicateKind.Inner
    ...
    )
    {
        ...
        for (var i = 0; i < files.Length; i++)
        {
            ...
            try
            {
                ...
                var host = !string.IsNullOrEmpty(outerMod.ModId)
                    ? outerMod.ModId
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(innerMod.ModId))
                        {
                            entries.Add(new()
                            {
                                ...
                                Name = !string.IsNullOrEmpty(innerMod.Name) ? innerMod.Name : innerMod.ModId,
