...
public partial class ExhibitPackageModal : Modal
{
    ...
    public static readonly DirectProperty<ExhibitPackageModal, bool> IsFavoriteProperty =
        AvaloniaProperty.RegisterDirect<ExhibitPackageModal, bool>(nameof(IsFavorite),
                                                                   o => o.IsFavorite,
                                                                   (o, v) => o.IsFavorite = v);
    ...
    private LazyObject ConstructChangelog()
    {
        var lazy = new LazyObject(async t =>
        {
            ...
            var description =
                await DataService.ReadChangelogAsync(new(Package.Label, Package.Namespace, Package.ProjectId, vid));
            return description;
        });
        return lazy;
    }
    ...
    private LazyObject ConstructDependencies()
    {
        var lazy = new LazyObject(async t =>
        {
            ...
            var vid = SelectedVersionMode == 0 ? SelectedVersion?.VersionId : null;
            var package =
                await DataService.ResolvePackageAsync(new(Package.Label, Package.Namespace, Package.ProjectId, vid),
                                                      Filter);
            var tasks = package
