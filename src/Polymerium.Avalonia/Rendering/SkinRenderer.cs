...
public sealed class SkinRenderer
{
    ...
    private SKImage RenderCover(SKBitmap skin)
    {
        ...
        // 上半身理想比例为 0.625（头+躯干 2.5 / 全身 4.0），再收 15% 让截断点落在大腿根部上方，
        // 少露腿，更像框住上半身的证件照。
        ...
        SKRectI? crop = null)
    {
        ...
        // WARNING: 本体先画（远→近）、外层后画（远→近叠在本体之上）；Nearest 采样保留像素艺术锐边，
    ...
    {
        // WARNING: SKShader.CreateBitmap 的纹理坐标是像素坐标（直接索引 bitmap），无需归一化。
        var uvs = new[]
        {
    ...
    // WARNING: 屏幕空间有符号面积（Skia Y 向下）：< 0 视为正面朝相机（与 CubeModel 顶点序匹配）。
