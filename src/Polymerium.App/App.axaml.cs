...
public class App : Application
{
    ...
    private static object? ActivatePage(Type view, object? parameter)
    {
        ...
        {
            // 避免又产生异常而导致无限循环
            if (activatorErrorCount++ < 3) return ActivatePage(typeof(PageNotReachedView), ex.Message);
        ...
        {
            // 避免又产生异常而导致无限循环
            if (activatorErrorCount++ < 3) return ActivatePage(typeof(ExceptionView), ex);
    ...
    {
        if (Program.AppHost is null) return new();
