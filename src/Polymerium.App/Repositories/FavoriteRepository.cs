...
public class FavoriteRepository(PersistenceService persistenceService, DataService dataService) : IRepository
{
    ...
    public async Task<RepositoryStatus> CheckStatusAsync()
    {
        ...
        var index = await dataService.GetMinecraftVersionsAsync().ConfigureAwait(false);
