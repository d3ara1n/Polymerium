    ...
    MinecraftService minecraftService) : ViewModelBase
{
    ...
    {
        if (model != null) overlayService.PopModal(new AccountEntryModal { DataContext = model });
    }
    ...
    private void MarkAsDefaultAccount(AccountModel? model)
    {
        if (model != null)
        {
            ...
            foreach (var account in Accounts) account.IsDefault = account.Uuid == model.Uuid;
        }
    }
