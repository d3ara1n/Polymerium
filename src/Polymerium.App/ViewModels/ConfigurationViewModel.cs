...
public class ConfigurationViewModel : ViewModelBase
{
    ...
    private bool CanOpenExportWizard() => false;

    private void OpenExportWizard()
    {
    }

    private bool CanChooseJava() => IsJvmHomeOverridden;
    ...
    private bool IsKeyOverridden(string key) => Model.Inner.Overrides.ContainsKey(key);
    ...
    private bool CanDoHarmToInstance(string? entered) =>
        SafeCodeGenerated == entered && _instanceService.CanManipulate(Model.Key);
    ...
    private void DeleteInstance(string? _)
    {
        ...
        var toDestroy = Model
            ...
            .SelectMany(x => x.Loaders)
            .Where(x => x.Identity == Loader.COMPONENT_BUILTIN_STORAGE)
            .Where(x => storages.All(y => x.Version != y.Version));
