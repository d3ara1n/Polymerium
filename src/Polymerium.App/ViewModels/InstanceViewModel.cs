...
public partial class InstanceViewModel : ViewModelBase
{
    ...
    protected override async Task OnInitializeAsync(CancellationToken token)
    {
        ...
        foreach (var widget in Context.Widgets) await widget.InitializeAsync();
    }
...

    {
        ...
        foreach (var widget in Context.Widgets) await widget.DeinitializeAsync();
    }
    ...
    {
        if (tracker.Key != Basic.Key) return;
    ...
    [ObservableProperty] public partial InstanceSubpageEntryModel? SelectedPage { get; set; }

    [ObservableProperty] public partial InstanceState State { get; set; } = InstanceState.Idle;
