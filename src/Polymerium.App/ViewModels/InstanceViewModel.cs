...
public class InstanceViewModel : ViewModelBase
{
    ...
    private string GetHomeFolderPath() => _context.InstanceHomePath(Profile.Key);

    private string GetAssetFolderPath(AssetKind kind) =>
        Path.Combine(GetHomeFolderPath(), FileNameHelper.GetAssetFolderName(kind));

    private bool CanOpenHomeFolder() => Directory.Exists(GetHomeFolderPath());

    private void OpenHomeFolder() => UriFileHelper.OpenInExternal(GetHomeFolderPath());
    ...
    private void OpenAssetFolder(AssetKind kind) => UriFileHelper.OpenInExternal(GetAssetFolderPath(kind));

    public void AddTodo(string text) => Profile.Todos.Add(new TodoModel(new Profile.RecordData.Todo(false, text)));

    private bool CanDeleteTodo(TodoModel? item) => item != null;
    ...
    public IList<AccountModel> GetAccountCandidates() =>
        _accountManager.Managed.Select(x =>
