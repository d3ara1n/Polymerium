...
public partial class InstancePropertiesViewModel : InstanceViewModelBase
{
    ...
    {
        if (_owned != null && _owned.Value.Overrides.TryGetValue(key, out var result) && result is bool rv) return rv;
    ...
    {
        if (ProfileManager.TryGetMutable(Basic.Key, out var guard)) _owned = guard;
    ...
    {
        if (_owned != null) await _owned.DisposeAsync();
    ...
    {
        if (_owned != null) _owned.Value.Setup.Source = null;
    ...
    [ObservableProperty] public required partial Bitmap ThumbnailOverwrite { get; set; }

    [ObservableProperty] public required partial string NameOverwrite { get; set; }

    [ObservableProperty] public partial string SafeCode { get; set; }

    #endregion

    #region Overrides

    [ObservableProperty] public partial string JavaHomeOverride { get; set; } = string.Empty;

    [ObservableProperty] public partial string JavaHomeWatermark { get; set; } = string.Empty;

    partial void OnJavaHomeOverrideChanged(string value) => WriteOverride(Profile.OVERRIDE_JAVA_HOME, value);

    [ObservableProperty] public partial string JavaMaxMemoryOverride { get; set; } = string.Empty;

    [ObservableProperty] public partial string JavaMaxMemoryWatermark { get; set; } = string.Empty;
    ...
    [ObservableProperty] public partial string JavaAdditionalArgumentsOverride { get; set; } = string.Empty;

    [ObservableProperty] public partial string JavaAdditionalArgumentsWatermark { get; set; } = string.Empty;
    ...
    [ObservableProperty] public partial string WindowInitialHeightOverride { get; set; } = string.Empty;

    [ObservableProperty] public partial string WindowInitialHeightWatermark { get; set; } = string.Empty;

    partial void OnWindowInitialHeightOverrideChanged(string value) =>
        WriteOverride(Profile.OVERRIDE_WINDOW_HEIGHT,
            !string.IsNullOrEmpty(value) && uint.TryParse(value, out var ui) ? ui : null);

    [ObservableProperty] public partial string WindowInitialWidthOverride { get; set; } = string.Empty;

    [ObservableProperty] public partial string WindowInitialWidthWatermark { get; set; } = string.Empty;

    partial void OnWindowInitialWidthOverrideChanged(string value) =>
        WriteOverride(Profile.OVERRIDE_WINDOW_WIDTH,
            !string.IsNullOrEmpty(value) && uint.TryParse(value, out var ui) ? ui : null);

    [ObservableProperty] public partial bool BehaviorResolveDependency { get; set; }
    ...
    [ObservableProperty] public partial bool BehaviorDeployFastMode { get; set; }
