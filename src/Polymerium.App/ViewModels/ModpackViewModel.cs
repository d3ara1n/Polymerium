...
public class ModpackViewModel : ViewModelBase
{
    ...
    public ModpackViewModel(RepositoryAgent repositoryAgent, TaskService taskService,
        NotificationService notificationService, IHttpClientFactory factory, ModpackExtractor extractor, ILogger<ModpackViewModel> logger)
    {
    ...
    private void InstallModpack(ModpackVersionModel? version)
    {
        if (version != null)
        {
            var task = new InstallModpackTask($"{Project.Inner.Label}:{Project.Inner.Id}/{version.Inner.Id}");
    ...
    {
        _logger.LogInformation("Start install modpack {project}({version}) from {repo}", project.Id, version.Id, project.Label);
        task.OnDownload();
        ...
        memory.Position = 0;
        task.OnExtract();
        var result = await _extractor.ExtractAsync(memory, (project, version), App.Current.Token);
        if (result.IsSuccessful)
        {
            var container = result.Value;
            _logger.LogInformation("Downloaded extracted modpack {name} ready to solidify", container.Original.Name);
            task.OnExport();
        ...
        else
        {
            ...
            var e = new ExtractException(result.Error, PurlHelper.MakePurl(project.Label, project.Id, version.Id));
            task.OnError(e);
            throw e;
        }
    }
