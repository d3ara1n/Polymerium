    ...
    DataService dataService) : ViewModelBase
{
    ...
    [ObservableProperty] public partial IReadOnlyList<MinecraftNewsModel>? TailNews { get; set; }

    [ObservableProperty] public partial MinecraftNewsModel? HeadNews { get; set; }
    ...
    {
        if (news != null) TopLevel.GetTopLevel(MainWindow.Instance)?.Launcher.LaunchUriAsync(news.ReadMoreLink);
