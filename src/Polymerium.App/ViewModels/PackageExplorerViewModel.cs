...
public partial class PackageExplorerViewModel : ViewModelBase
{
    ...
    private ExhibitModel LinkExhibit(Project project)
    {
        ...
        if (foundInPending != null) return foundInPending;

        var foundInResult = Exhibits?.FirstOrDefault(x => x.Label == project.Label
                                                          && x.Ns == project.Namespace
                                                          && x.ProjectId == project.ProjectId);
        if (foundInResult != null) return foundInResult;
        ...
        if (installed is not null)
        {
            ...
            if (PackageHelper.TryParse(installed.Purl, out var parsed)) model.InstalledVersionId = parsed.Vid;
        }
    ...
    [ObservableProperty] public partial InstanceBasicModel Basic { get; set; }

    [ObservableProperty] public partial RepositoryBasicModel SelectedRepository { get; set; }
    ...
    [ObservableProperty] public partial Filter Filter { get; set; } = Filter.None with { Kind = ResourceKind.Mod };

    [ObservableProperty] public partial string QueryText { get; set; } = string.Empty;

    [ObservableProperty] public partial bool IsFilterEnabled { get; set; }
    ...
    [ObservableProperty] public partial ResourceKind? SelectedKind { get; set; }

    partial void OnSelectedKindChanged(ResourceKind? value)
    {
        if (value != null) Filter = Filter with { Kind = value };
    }

    [ObservableProperty] public partial InfiniteCollection<ExhibitModel>? Exhibits { get; set; }
        ...
        if (_profileManager.TryGetMutable(Basic.Key, out var guard))
        {
            foreach (var model in PendingPackagesSource.Items)
                ...
                else if (model is { State: ExhibitState.Removing, Installed: not null })
                {
                    ...
                    if (exist != null) guard.Value.Setup.Packages.Remove(exist);
