...
public class PersistenceService(IFreeSql freeSql)
{
    ...
    public class Account(
        ...
        bool isDefault)
    {
        [Column(IsPrimary = true)] public string Uuid { get; set; } = uuid;

        public string Kind { get; set; } = kind;

        [Column(DbType = "BLOB")] public string Data { get; set; } = data;
    ...
    {
        [Column(IsPrimary = true)] public string Key { get; set; } = key;
    ...
    public class WidgetLocalSection(string key, string widgetId, string indicator, string data)
    {
        ...
        [Column(IsPrimary = true)] public string WidgetId { get; set; } = widgetId;

        [Column(IsPrimary = true)] public string Indicator { get; set; } = indicator;
    ...
    public double GetPercentageInTotalPlayTime(string key)
    {
        ...
        if (allTotalSeconds == 0) return 0d;
