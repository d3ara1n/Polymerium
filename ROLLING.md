...
Each version file follows the Keep a Changelog convention with the four sections Added, Changed, Removed, and Fixed, plus a ✨ Highlights ✨ block at the top that repeats verbatim up to three entries from the sections below that carry real value to end users — meaning a change most users will directly perceive, not a behind-the-scenes tweak that only surfaces under specific troubleshooting paths, without rewriting or rephrasing them and dropping any trailing issue key or attribution, and stays a single bare dash when nothing below clears that bar.
