...
| Native patches (Trident Portable Instance only) | ✅ — the import patch layer; locally managed user patches are never exported |
...
- **Trident Portable Instance (TPI)** preserves the most information: tags, deployment rules, account bindings, native patches, and extra files. Best for Polymerium-to-Polymerium sharing.
...
Trident Portable Instance (TPI) is fundamentally different from other modpack formats. Rather than being just a mod list, it's a **complete instance snapshot** — a portable representation of everything Polymerium knows about an instance.

### What's Inside

A TPI `.zip` archive contains:

| File | Purpose |
|------|---------|
| `trident.index.json` | Full instance profile: mod list with Prefs, tags, rules, overrides, source tracking |
...
TPI preserves dimensions of data that other formats cannot represent:
...
This makes TPI the only format that can faithfully reproduce an instance's full configuration — not just its mod list.
