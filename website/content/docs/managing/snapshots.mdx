...
If Local Data and the Run Directory hold different ordinary files at the same path, a snapshot does not include the unreconciled Run Directory copy. Complete a deploy and keep a separate backup first. See [Known Issues](/docs/appendix/known-issues) for this and the interruption boundary during restore.
