...
Polymerium resolves the Java majors an instance accepts against the paths you configured. If none of your configured paths covers any accepted major, it downloads a matching runtime during deploy. This happens once — the runtime is cached and reused.
...
A modpack can narrow the accepted majors through a native patch. If nothing you configured covers the accepted majors, or the accepted set is empty, the launch fails with a prompt to open Java settings.
