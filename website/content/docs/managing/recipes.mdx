...
Applying a recipe does not change the instance's Minecraft version or loader. Its project references resolve available package versions when you apply it. Unlike an in-place [collection](/docs/managing/collections), the recipe also exists independently of any one instance.
...
A collection on the same Setup page is only a local group. Use its header menu to **Promote to Recipe** when you want to maintain a separate reusable list. To remove an instance group's recipe association without deleting its packages or the recipe itself, choose **Demote to Collection** on the recipe group. Trident modpack export represents local recipe groups as collections, because the local recipe library does not travel with a modpack.
