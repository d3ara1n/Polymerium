...
Files from your `import/` and `persist/` directories are also layered into `build/` in priority order — import as real files, persist as symlinks. Runtime edits to imported content stay in `build/` as the working copy. See [Instance Directory Model →](/docs/concepts/four-layers) for how that works.
...
- [Instance Directory Model →](/docs/concepts/four-layers) — how files are layered into the game directory
