...
The game directory (`build/`) is a build artifact. You can delete it and redeploy at any time. For files you can't afford to lose — like `options.txt` or your saves — move them into the `persist/` directory. Polymerium never touches `persist/` during resets or updates. See [Instance Directory Model →](/docs/concepts/four-layers#persist---user-managed-persistence) for details.
...
- [Instance Directory Model →](/docs/concepts/four-layers) — understand how files are organized inside an instance
