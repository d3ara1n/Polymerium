...
The game directory (`build/`) is a build artifact. You can delete it and redeploy at any time. For files you can't afford to lose — like `options.txt` or your saves — move them into the `persist/` directory. Polymerium never touches `persist/` during resets or updates. See [Directory Model →](/docs/concepts/directory-model#persist---user-managed-persistence) for details.
...
- [Directory Model →](/docs/concepts/directory-model) — understand how files are organized inside an instance
- [Deploy Pipeline →](/docs/concepts/deploy-explained) — see exactly what happens when you click "Deploy"
- [Native Patches →](/docs/advanced/patches) — extend an instance's launch plan with external patch documents
