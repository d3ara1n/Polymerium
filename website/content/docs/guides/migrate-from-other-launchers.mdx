...
3. Paste them into your Polymerium instance's `persist/` directory. For directories like `saves/`, create a `.keep` file inside (`persist/saves/.keep`) so they get linked into the game. See [Instance Directory Model →](/docs/concepts/four-layers#persist---user-managed-persistence) for details.
