...
2. Move the shader `.zip` to `persist/shaderpacks/` and create `persist/shaderpacks/.keep` so the directory is linked into the game. See [Instance Directory Model →](/docs/concepts/four-layers#persist---user-managed-persistence) for how `persist/` works.
