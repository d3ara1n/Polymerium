...
2. Changes land on the working copy in `build/` (untracked).
3. Open the **Workspace** tab to review diffs.
4. Promote desired changes from the working copy → `import/`.
5. Commit:
...
- **Don't track `build/` or `persist/`** — these are personal and machine-specific.
- **Use the workspace** for promoting working-copy changes to `import/` — it shows exactly what changed.
