...
2. Changes land in `build/` (untracked).
3. Open the **Workspace** tab to review diffs.
4. Sync desired changes from `build/` → `import/`.
5. Commit:
...
- **Use the workspace** for promoting changes from `build/` to `import/` — it shows exactly what changed.
