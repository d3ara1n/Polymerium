...
- `build/` 是可能包含个人数据的运行目录，不应纳入 Git；需要时请单独备份。`persist/` 是本地保留，应保持私有。
...
| `import/` | ✅ | 可分发文件（配置、资源） |
| 导入层原生 Patch（`patches/data.patch.json`、`patches/import/`） | ✅ 使用时 | 可随 Trident 整合包分发的启动声明 |
| `build/` | ❌ | 运行产物，可能含有尚未备份的存档等数据 |
| `persist/` | ❌ | 玩家数据（存档、截图） |
| `data.lock.json` | ❌ | 部署缓存，自动生成 |

## 开发工作流

实例首页的开发概览组件汇总 Git 状态、整合包源文件、未同步的工作副本变更、Patch、发行附件及上次运行结果。紧凑形态会隐藏无事可报的类别。要检查具体文件变更，请打开[工作区](/docs/advanced/workspace)。
