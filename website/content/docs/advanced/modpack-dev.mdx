...
- `build/` is the Run Directory; it can contain personal data, so keep it out of Git and back it up separately when needed. `persist/` is Local Data and should stay private.
...
| Imported native patches (`patches/data.patch.json`, `patches/import/`) | ✅ If used | Launch declarations that travel with a Trident modpack |
| `build/` | ❌ | Runtime output; may contain saves and other data not backed up elsewhere |
| `persist/` | ❌ | Player data (saves, screenshots) |
| `data.lock.json` | ❌ | Deploy cache, auto-generated |

## Development Workflow

On the instance home page, the development overview widget summarizes Git status, Pack Source files, unsynced working-copy changes, patches, release attachments, and the last run. Its compact view hides categories with nothing to report. Open [Workspace](/docs/advanced/workspace) to review individual file changes.

### Iterating on Configs

1. Launch the game and modify settings in-game.
2. Changes land in `build/`.
...
3. Each person's `build/` and `persist/` are local and untracked.
...
- **Use the workspace** as your staging area — it's purpose-built for promoting in-game changes from `build/` into `import/`.
