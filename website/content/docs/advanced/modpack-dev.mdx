...
- `build/` and `persist/` are runtime artifacts that don't belong in version control
...
2. Changes land on the working copy in `build/` (import projection).
...
3. Each person's `build/` working copy and `persist/` are local and untracked.
...
- **Use the workspace** as your staging area — it's purpose-built for promoting the working copy into `import/`.
