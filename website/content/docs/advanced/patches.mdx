...
Libraries can use a download URL or a local file relative to the patch document. Local files must accompany the patch; they are instance data rather than disposable shared cache entries. Native libraries are re-extracted when their contents, order or exclusion rules change, including when switching to an older version.
