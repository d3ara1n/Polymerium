...
- Before using **Reset** or deleting `build/`, back up any saves, screenshots, or configuration stored only in the Run Directory. Reset permanently clears it; only files already in Local Data (`persist/`) are protected. See [Directory Model](/docs/concepts/directory-model).
