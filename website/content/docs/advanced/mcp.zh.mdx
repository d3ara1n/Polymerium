---
title: MCP 模式与 AI Agent 集成
description: 让 AI Agent 通过模型上下文协议管理你的 Minecraft 实例——Polymerium 独有功能。
---

## 什么是 MCP？

模型上下文协议（Model Context Protocol，MCP）是 AI Agent 与外部工具交互的标准。Polymerium 的 CLI（`trident`）可以作为 MCP 服务器运行，暴露 30 多个工具供 AI Agent 直接调用。
