...
| **文件组织** | 整合包源（`import/`）/ 本地保留（`persist/`）/ 运行目录（`build/`） | 单一 `.minecraft` 文件夹 |
| **重置/重建** | 清除运行目录，再从元数据重新部署 | 手动删除文件/重新导入 |
...
├── import/    ← 整合包源
├── persist/   ← 你明确保留的本地数据
└── build/     ← Minecraft 实际读写的运行目录
```

这种分离意味着：
- 整合包更新不会覆盖你已经放入本地保留（`persist/`）的数据
- 重置会删除只存在于运行目录（`build/`）中的存档，世界需要先移动到 `persist/saves/` 才会受到保护
- 运行目录中的可复现游戏文件可以从元数据和缓存重新构建
