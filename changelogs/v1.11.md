...
- Rework the settings page layout into grouped card sections
- Rework the instance properties page layout into grouped card sections

### Fixed

- Fix swapped window initial width and height fields in game defaults
...
- Fix a crash that closed the launcher when an instance deployment failed instead of showing the failure
- Fix popup panels on macOS appearing as opaque rectangles without rounded corners or shadow (Huskui.Avalonia)
- Fix instances created from imported modpacks ignoring the custom name entered on creation and keeping the modpack name (#POLY-135)
